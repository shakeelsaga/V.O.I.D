...
|---|---|---|
| `v2.3.0` | 2026-05-11 | Hop-distance vector routing, passive neighbor table, RSSI-composite peer scoring (ESP32), ESP32 promiscuous RSSI sidecar, ESP8266 baseband limitation workaround, ACK encryption fix, duplicate suppression lockout fix, `hops` in MQTT output |
...
### V2.3.0 - Released
- [x] `hopDist` field in `SurvivorPayload` (offset 10, 11-byte struct)
- [x] 5-entry passive neighbor table (RSSI + hopDist, zero probe traffic)
- [x] RSSI-composite peer scoring with `RSSI_FLOOR` disqualification
- [x] Neighbor table fast-path in `huntForPeers()`
- [x] `myHopDist` self-estimate, updated on every confirmed TX
- [x] `"hops"` field in MQTT JSON output and DTN flash buffer
- [x] ESP32 promiscuous RSSI sidecar (disabled on ESP8266 - hardware incompatible)
- [x] ACK-before-duplicate-check fix for rebooted node lockout
- [x] LMK encryption enforced on all relay ACK paths
- [x] Hardware field verification - 2-hop relay confirmed stable

### V2.4.0 - Planned
- [ ] Probabilistic gossip forwarding for non-SOS relay (`GOSSIP_PROB = 75`) - reduces relay flood ~35%
- [ ] DTN bundle expiry (`expiresAt` timestamp in flash buffer, aligns with RFC 9171)
- [ ] Backup peer slot in neighbor table (primary + fallback, no re-scan on primary miss)
